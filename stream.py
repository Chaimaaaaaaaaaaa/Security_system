# Basé sur : Dhairya Parikh, "How To Live Stream The Raspberry Pi Camera (2 Easy Ways)"
# Disponible sur : https://raspberrytips.com/how-to-live-stream-pi-camera/
...
class StreamingHandler(server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == '/stop':
            # Stop the streaming server when the "Stop Stream" button is pressed
            self.send_response(200)
            self.send_header('Content-Type', 'text/html')
            self.end_headers()
            self.wfile.write(b"Stream stopped. The server is shutting down.")
